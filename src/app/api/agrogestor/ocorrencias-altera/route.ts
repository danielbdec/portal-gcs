...
// URL do webhook n8n para alteração de ocorrências
const EXTERNAL_API_URL = 'http://localhost:5678/webhook/agrogestor-altera-ocorrencias';
...
export async function POST(request: Request) {
    try {
        ...
        if (!body.id || !body.empreendimento_id || !body.descricao) {
        ...
        if (!isValid) {
            ...
            return NextResponse.json(
                { error: 'Ocorreu um erro ao processar a alteração. Resposta inesperada.' },
                { status: 502 }
            );
        }

        // Se passou na validação, retorna o primeiro item
        return NextResponse.json(data[0]);

    } catch (error) {
        console.error('Erro interno no servidor (Alteração):', error);
        return NextResponse.json(
