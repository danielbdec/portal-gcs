...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Define o endpoint do webhook de consulta de variedades
    const webhookUrl = "http://localhost:5678/webhook/gestao-variedade-consulta";
    ...
    if (!webhookResponse.ok) {
      ...
      console.error(`[API /consulta-variedade] Webhook retornou erro ${webhookResponse.status}:`, errorText);
      return new NextResponse(errorText || "O serviço de consulta de variedade retornou um erro.", { status: webhookResponse.status });
    }
    ...
    console.error("[API /consulta-variedade] Erro no handler da rota:", e);
