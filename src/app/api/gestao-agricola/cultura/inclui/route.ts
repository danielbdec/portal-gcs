...
const WEBHOOK_URL = 'http://localhost:5678/webhook/gestao-variedade-inclui';

/**
 * Handler para POST - Incluir uma nova Variedade
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('[API /inclui-variedade] Erro ao parsear JSON da requisição:', error);
  ...
  if (!registro) {
    console.error('[API /inclui-variedade] Requisição recebida sem o objeto "registro".');
    return NextResponse.json({ message: 'Dados do "registro" ausentes.' }, { status: 400 });
  }

  console.log('[API /inclui-variedade] Recebido para inclusão:', registro);
  ...
  try {
    ...
    if (!fetchResponse.ok) {
      ...
      console.error(`[API /inclui-variedade] Webhook retornou erro ${fetchResponse.status}:`, responseData);
    ...
    console.log('[API /inclui-variedade] Resposta do Webhook:', responseData);
    return NextResponse.json(responseData, { status: 200 }); // Retorna 200 (OK) em vez de 201 (Created) para manter padrão

  } catch (error: any) {
    // Captura erros de rede (ex: webhook offline) ou erros de parse do JSON do webhook
    console.error('[API /inclui-variedade] Erro ao contatar o webhook:', error.message);
    return NextResponse.json(
