...
const ModalEmpreendimento = ({
    ...
    // ================== ALTERAÇÃO PARA DEIXAR MAIÚSCULO ==================
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'cnpj_cpf') {
            ...
            // Converte para maiúsculas para os campos de texto
            finalValue = value.toUpperCase();
        }
        // Para outros campos como 'estado' (select), o valor original é mantido
    ...
    const handleSaveClick = () => {
        ...
        if (mode !== 'delete') {
            if (!formData.nome || !formData.cnpj_cpf || !formData.unidade || !formData.estado) {
        ...
        <>
            ...
            <div
                ...
                style={{
                    ...
                    zIndex: 1001, width: '90%', maxWidth: '700px',
                ...
                <div style={{ padding: '1.5rem' }}>
                    {isDeleteMode ? (
                        <div style={{textAlign: 'center'}}>
                            <AlertTriangle size={40} color="#F58220" style={{marginBottom: '1rem'}}/>
                            <p style={{ color: '#333', fontSize: '1.1rem' }}>
                                Tem certeza que deseja excluir o empreendimento?
                            </p>
                            <p style={{fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--gcs-red)', background: '#fff0f0', padding: '8px', borderRadius: '6px', marginTop: '1rem' }}>
                                {formData.nome}
                            </p>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                            <div style={{ gridColumn: '1 / -1' }}>
                                ...
                                <input className="modal-input" name="nome" value={formData.nome || ''} onChange={handleInputChange} disabled={isSaving} />
                            </div>
                            <div>
                                <label className="modal-label">CPF / CNPJ *</label>
                                <input className="modal-input" name="cnpj_cpf" value={formData.cnpj_cpf || ''} onChange={handleInputChange} disabled={isSaving} placeholder="Digite o CPF ou CNPJ" />
                            </div>
                             <div>
                                <label className="modal-label">Número da Matrícula</label>
                                <input className="modal-input" name="numero_matricula" value={formData.numero_matricula || ''} onChange={handleInputChange} disabled={isSaving} />
                            </div>
                            <div>
                                <label className="modal-label">Unidade (Fazenda) *</label>
                                <input className="modal-input" name="unidade" value={formData.unidade || ''} onChange={handleInputChange} disabled={isSaving} />
                            </div>
                            <div>
                                <label className="modal-label">Estado *</label>
                                <select className="modal-input" name="estado" value={formData.estado || ''} onChange={handleInputChange} disabled={isSaving}>
