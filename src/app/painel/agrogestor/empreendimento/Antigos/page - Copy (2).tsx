...
export default function AgrogestorPage() {
  ...
  const fetchEmpreendimentos = async () => {
      ...
      try {
        ...
        const empreendimentosComChave = data.map(emp => ({
          ...emp,
          key: emp.id.toString(),
        }));
        ...
        setEmpreendimentos([]);
        alert("Não foi possível carregar os empreendimentos. Verifique o console para mais detalhes.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveEmpreendimento = async (data: Partial<Empreendimento>, mode: 'add' | 'edit' | 'delete') => {
      ...
      try {
        ...
        switch (mode) {
          case 'add':
            response = await fetch('/api/agrogestor/inclui-empreendimento', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            if (!response.ok || result.status !== 'ok') {
              throw new Error(result.message || 'Ocorreu um erro ao incluir o empreendimento.');
            }
            alert(result.message || "Empreendimento incluído com sucesso!");
            break;

          case 'edit':
            response = await fetch('/api/agrogestor/altera-empreendimento', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(data),
            });
            result = await response.json();
            if (!response.ok || result.status !== 'ok') {
              throw new Error(result.message || 'Ocorreu um erro ao alterar o empreendimento.');
            }
            alert(result.message || "Empreendimento alterado com sucesso!");
            break;

          case 'delete':
            response = await fetch('/api/agrogestor/exclui-empreendimento', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            result = await response.json();
            if (!response.ok || result.status !== 'ok') {
              throw new Error(result.message || 'Ocorreu um erro ao excluir o empreendimento.');
            }
            alert(result.message || "Empreendimento excluído com sucesso!");
            break;
        }
