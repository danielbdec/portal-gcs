...
export default function AgrogestorPage() {
  ...
  // ===================== FUNÇÃO DE SALVAR CORRIGIDA PARA LER A RESPOSTA EM ARRAY =====================
  const handleSaveEmpreendimento = async (data: Partial<Empreendimento>, mode: 'add' | 'edit' | 'delete') => {
      ...
      try {
        ...
        // Validação correta: checa se a resposta é um array, se não está vazio e se o primeiro item tem status 'ok'
        const isSuccess = response.ok && Array.isArray(result) && result.length > 0 && result[0].status === 'ok';

        if (isSuccess) {
            alert(result[0].message || "Operação realizada com sucesso!");
        } else {
