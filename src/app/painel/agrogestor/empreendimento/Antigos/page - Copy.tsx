...
export default function AgrogestorPage() {
  ...
  const kpiData = useMemo(() => {
    ...
    // A API de consulta ainda não retorna os dados de status_docs, então zeramos o valor.
  ...
  const handleSaveEmpreendimento = async (data: Partial<Empreendimento>, mode: 'add' | 'edit' | 'delete') => {
      ...
      try {
        if (mode === 'add') {
            const response = await fetch('/api/agrogestor/inclui-empreendimento', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            alert(`Empreendimento ${data.id} alterado com sucesso! (Simulado)`);
        } else if (mode === 'delete') {
            await new Promise(resolve => setTimeout(resolve, 1500));
            alert(`Empreendimento ${data.id} excluído com sucesso! (Simulado)`);
        }
