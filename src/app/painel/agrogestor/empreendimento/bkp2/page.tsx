...
export default function AgrogestorPage() {
  ...
  const handleExportXLSX = () => {
    const headers = ["ID", "Empreendimento", "Matrícula", "CPF/CNPJ", "Unidade", "Estado", "Área (ha)", "Situação"];
    const data = empreendimentosFiltrados.map(emp => [
        emp.id, emp.nome, emp.numero_matricula, emp.cnpj_cpf, emp.unidade, emp.estado, emp.area, emp.situacao
    ]);
    const worksheet = XLSX.utils.aoa_to_sheet([headers, ...data]);
    worksheet['!cols'] = [ { wch: 5 }, { wch: 60 }, { wch: 20 }, { wch: 20 }, { wch: 25 }, { wch: 15 }, { wch: 12 }, { wch: 16 }];
  ...
  // --------- Mapeamento de cor por situação (bolinha) ----------
  const getSituacaoColor = (situacao?: string) => {
    const s = (situacao || '').toString().trim().toUpperCase();
    if (s === 'ATIVO') return 'var(--gcs-green)';         // verde
    if (s === 'ARRENDO') return '#F2C94C';                // amarelo
    if (s === 'LITIGIO') return 'var(--gcs-orange)';      // laranja
    if (s === 'INATIVO') return 'var(--gcs-red)';         // vermelho
    return '#BDBDBD';                                     // cinza (desconhecido)
  };
  ...
  const columns = [
    { title: "ID", dataIndex: "id", key: "id", width: 80, sorter: (a: Empreendimento, b: Empreendimento) => a.id - b.id },
    {
      ...
      render: (_: any, record: Empreendimento) => {
        const color = getSituacaoColor(record.situacao);
        const label = (record.situacao || '').toString().trim() || 'INDEFINIDO';
        return (
          <Tooltip title={label}>
            <div
              style={{
    ...
    { title: "Área (ha)", dataIndex: "area", key: "area", width: 120 },
    // (Removida a coluna textual "Situação" da posição antiga para evitar duplicidade)
    {
      title: "Ações",
      key: "action",
      align: "center" as const,
