...
export interface Empreendimento {
  ...
  area_preservacao_permanente?: number; // NOVO CAMPO
}
...
const FormFields = ({ formData, mode, disabled, handleInputChange }: {
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1rem' }}>
        ...
        <div style={{ gridColumn: '1 / 2' }}>
            <label className="modal-label">Área de Preservação Permanente (APP) (ha)</label>
            <input 
                type="number"
                className="modal-input" 
                name="area_preservacao_permanente" 
                value={formData.area_preservacao_permanente ?? ''} 
                onChange={handleInputChange} 
                disabled={disabled} 
                placeholder="Ex: 10.5"
            />
...
const ModalEmpreendimento = ({
    ...
    useEffect(() => {
        if (visible && initialData) {
            setFormData({
              ...
              // Mapeia o campo da API (area_preserv_perm) para o campo do formulário
              area_preservacao_permanente: (initialData as any).area_preserv_perm,
            });
            ...
            setFormData({
                ...
                area: undefined,
                situacao: '',
                area_agricultavel: undefined,
                area_reserva_legal: undefined,
                area_preservacao_permanente: undefined,
            });
        }
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        } else if (name === 'area' || name === 'area_agricultavel' || name === 'area_reserva_legal' || name === 'area_preservacao_permanente') {
