...
import { Tooltip, Button, Typography, Input, Table, message, Empty } from "antd";
...
// NOTA: Idealmente, este hook ficaria em seu próprio arquivo (ex: utils/useDebouncedValue.ts)
...
export default function AgrogestorPage() {
  ...
  const fetchEmpreendimentos = useCallback(async () => {
    ...
    try {
        ...
        if (e.name !== 'AbortError') {
            message.error(e?.message ?? "Falha ao carregar empreendimentos.");
        }
    } finally {
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      if (initialFetchDone.current) return; // Garante que a lógica rode apenas uma vez

      const user = session.user;
      const hasAccess = user?.is_admin === true || user?.funcoes?.includes('agrogestor.empreendimentos');
      
      if (hasAccess) { 
        initialFetchDone.current = true; // Marca como executado para não rodar de novo
  ...
  const empreendimentosFiltrados = useMemo(() => {
    ...
    if (advancedFilters.estado !== ALL_STATES) {
      dados = dados.filter(e => e.estado === advancedFilters.estado);
    }
    if (advancedFilters.unidade !== ALL_UNITS) {
      dados = dados.filter(e => e.unidade === advancedFilters.unidade);
    }
    return dados;
  }, [empreendimentos, buscaDebounced, advancedFilters]);

  const handleExportXLSX = () => {
    const rows = empreendimentosFiltrados.map(e => ({
      ID: e.id ?? "",
    ...
    ws['!cols'] = [{wch:8},{wch:40},{wch:16},{wch:18},{wch:16},{wch:10}];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Empreendimentos");
    XLSX.writeFile(wb, "empreendimentos_gcs.xlsx");
  };
  
  const handleOpenGestaoModal = useCallback((e: EmpreendimentoCompleto) => {
    setSelectedEmpreendimento(e);
    setIsGestaoModalOpen(true);
  }, []);

  const handleCloseGestaoModal = useCallback(() => {
  ...
  return (<>
    <style>{`
        ...
        /* ESTILO PARA O CABEÇALHO DA TABELA E CONTAINER */
        ...
        /* ESTILO PARA QUEBRA DE LINHA E ZEBRADO */
      ...
      <div className="header-wrapper" style={{ display: 'flex', alignItems: 'stretch', gap: '1.5rem', marginBottom: '1.5rem' }}>
        <div className="chart-card" style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
            ...
            <div style={{ width: 280, height: 160 }} aria-label="Distribuição de empreendimentos por estado">
                {!hasLoadedOnce ? (
                     <div style={{height:160, display:'flex',alignItems:'center',justifyContent:'center'}}><LoadingSpinner text="" /></div>
                ) : dadosGraficoEstados.length === 0 ? (
                    <div style={{height:160, display:'flex',alignItems:'center',justifyContent:'center', color: 'var(--gcs-gray-dark)'}}><span>Nenhum dado para exibir</span></div>
                  ) : (
        ...
        <div className="main-content-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <Input aria-label="Buscar empreendimentos" placeholder="Buscar por nome ou CNPJ/CPF..." value={busca} onChange={(e) => setBusca(e.target.value)} style={{ padding: "12px 16px", width: "350px", borderRadius: "8px" }}/>
      ...
      <div className="content-card" aria-busy={loading}>
        ...
        {!hasLoadedOnce ? (
          ...
          <Table
            ...
            loading={{ spinning: loading, tip: 'Atualizando...', size: 'large' }}
            sticky
            size="middle"
            pagination={{ pageSize: 10, showSizeChanger: true, position: ["bottomRight"] }}
            scroll={{ x: 1200, y: 520 }}
            locale={{ 
                emptyText: (empreendimentos.length > 0 && empreendimentosFiltrados.length === 0) ? (
                    <Empty description="Nenhum resultado encontrado para os filtros aplicados.">
                        <Button type="primary" onClick={handleClearAllFilters}>Limpar Filtros</Button>
                    </Empty>
                ) : "Nenhum dado para exibir."
            }}
            rowClassName={(_, index) => index % 2 === 0 ? 'row-even' : 'row-odd'}
            onRow={(record) => ({
              onDoubleClick: () => handleOpenGestaoModal(record),
            })}
          />
        )}
      </div>
