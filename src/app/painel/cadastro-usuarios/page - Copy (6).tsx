...
// Interface para a estrutura que a API de funções retorna
...
interface Usuario {
  ...
  status: string;
  is_admin: boolean;
  funcoes: number[]; // Agora esperamos que seja sempre um array de números no final
}
...
function GerenciamentoUsuarioModal({ 
    ...
    useEffect(() => {
        if (isOpen && user) {
            ...
            // Lógica simplificada: agora o `user.funcoes` já vem como um array de números
...
function CadastroUsuariosInner() {
  ...
  const openModalForNew = () => {
    // Para um novo usuário, preenchemos com dados padrão e abrimos o modal
    const newUserTemplate: Usuario = {
      id: 0, // ID temporário ou nulo
      email: '',
      status: 'ativo',
      is_admin: false,
      funcoes: [], // Começa sem funções
    };
  ...
  const openModalForEdit = async (user: Usuario) => {
    // Mostra um feedback de carregamento se desejar (opcional)
    // ex: setIsLoading(true);

    try {
      // 1. Chama a nova API para buscar as funções do usuário específico
      const res = await fetch('/api/portal/consulta-usuarios-funcoes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: user.id }), // Envia o ID do usuário
      });
      ...
      // 2. Extrai apenas os IDs das funções para um array de números
      const funcoesIds = funcoesDoUsuario.map(f => f.funcao_id);

      // 3. Cria um objeto de usuário completo com as funções que acabamos de buscar
      const completeUser = {
        ...user,
        funcoes: funcoesIds,
      };

      // 4. Define o usuário completo no estado e abre o modal
      ...
      // Para o feedback de carregamento (opcional)
