...
export default function GestaoPlanejamentoPage() {
  ...
  const handleSave = async (data: { header: any, items: PlanejamentoItem[] }) => {
    ...
    // O backend tratará `data.header` (sem ID) e `data.items`
    // TODO: Usar endpoint 'gestao-planej-cabec-inclui' e 'gestao-planej-item-inclui' (em 'add')
    // TODO: Usar endpoint 'gestao-planej-cabec-altera' e 'gestao-planej-item-altera' (em 'edit')
    
    try {
      // Simulação
      await new Promise(resolve => setTimeout(resolve, 1500));
      console.log("Simulando salvamento (Tudo):", data);
      
      const successMessage = modalMode === 'add' 
        ? 'Plano salvo com sucesso (simulação).'
        : 'Plano alterado com sucesso (simulação).';

      setNotification({ visible: true, type: 'success', message: successMessage });
      handleCloseDetalheModal();
      ...
      setNotification({ visible: true, type: 'error', message: error.message || 'Não foi possível salvar o planejamento.' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (<>
      ...
      <div className="header-wrapper" style={{ display: 'flex', alignItems: 'stretch', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="main-content-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '1.5rem', paddingTop: '0.5rem', paddingBottom: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <Input
                    type="text"
                    placeholder="Buscar por filial, safra, descrição ou ID..." // ATUALIZADO
                    value={busca}
                  ...
                  {planejamentosPaginados.map((item) => {
                    ...
                    return (
                      <tr key={item.key} className="data-row">
                        ...
                        <td data-label="Filial" style={{ padding: '14px 12px', verticalAlign: 'middle' }}>{item.filial}</td> {/* NOVO */}
    ...
    <ModalPlanejamentoDetalhe
        ...
        onSave={handleSave} // (NOVO) Passa a função de salvar TUDO
        isSaving={isSaving} // (Renomeado) Passa o isSaving da página
    />
