...
const FilterPopover = ({
    ...
    const handleApply = () => {
        onApplyFilters({ filial, tipo, responsavel });
        setIsOpen(false);
    };
    ...
    const handleClear = () => {
        ...
        onApplyFilters({ filial: 'Todas', tipo: 'Todos', responsavel: 'Todos' });
        setIsOpen(false);
    };
...
export default function ConsultaNotas() {
  ...
  const [advancedFilters, setAdvancedFilters] = useState({ filial: 'Todas', tipo: 'Todos', responsavel: 'Todos' });
  ...
  const notasFiltradasPorAvancado = useMemo(() => {
    return (notas || []).filter((nota) => {
        ...
        return filialOk && tipoOk && responsavelOk;
    });
  ...
  const statusCounts = useMemo(() => {
      ...
      Object.keys(statusNfMap).forEach(status => {
          counts[status] = source.filter(n => n.status_nf?.trim().toLowerCase() === statusNfMap[status]).length;
      });

      counts['Compras'] = source.filter(n => n.status_compras?.trim().toUpperCase() !== 'CONCLUÍDO').length;
      counts['Fiscal'] = source.filter(n => n.status_fiscal?.trim().toUpperCase() !== 'CONCLUÍDO').length;
  ...
  const areFiltersApplied = useMemo(() => {
    ...
    const isAdvancedFiltered = advancedFilters.filial !== 'Todas' || advancedFilters.responsavel !== 'Todos' || advancedFilters.tipo !== 'Todos';
