...
import { FaCheckCircle, FaTimesCircle, FaExclamationTriangle, FaListAlt, FaHistory, FaFileInvoice, FaSyncAlt, FaInfoCircle, FaUserFriends, FaSearch, FaTimes, FaSpinner, FaCopy, FaShieldAlt, FaMagic, FaPencilAlt } from "react-icons/fa";
...
const ManualTesModal = ({
    ...
    useEffect(() => {
        if (isOpen && tesData) {
            ...
            const initialTes = tesData.itens.reduce((acc, item) => {
                if(item.id) {
                    // CORREÇÃO: Usa tes_codigo como valor inicial
    ...
    const handleSave = () => {
        ...
        // CORREÇÃO: Validação de preenchimento obrigatório
        ...
        const originalTesState = tesData.itens.reduce((acc, item) => {
            if(item.id) acc[item.id] = item.tes_codigo; // Compara com tes_codigo
            return acc;
        }, {} as Record<number, string>);
        ...
        if (updates.length > 0) {
            ...
            // Se a validação passou mas não há alterações, apenas fecha.
            onClose();
        }
    };
    ...
    return (
        <>
            ...
            <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '2rem', borderRadius: '8px', zIndex: 2147483649, width: '90%', maxWidth: '900px', display: 'flex', flexDirection: 'column', gap: '1rem', maxHeight: '90vh' }}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0}}>
                    <h3 style={{ margin: 0, fontSize: '1.2rem' }}>Informar TES Manualmente</h3>
                ...
                <div style={{ overflowY: 'auto', flexGrow: 1, borderTop: '1px solid #eee', borderBottom: '1px solid #eee', padding: '1rem 0' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '1rem', marginTop: '1rem', flexShrink: 0, position: 'relative', minHeight: '40px' }}>
                    {apiMessage && (
                        <div style={{ position: 'absolute', left: '0', color: apiMessage.type === 'success' ? '#28a745' : '#dc3545', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            {apiMessage.type === 'success' ? <FaCheckCircle /> : <FaTimesCircle />}
                            {apiMessage.text}
                        </div>
                    )}
...
export default function ModalDetalhes({ chave, statusNF, visivel, onClose, nomeFornecedor, onActionSuccess, status_tes, statusCompras }: ModalDetalhesProps) {
  ...
  const [activeTab, setActiveTab] = useState<'itens' | 'historico' | 'erros' | 'fiscal' | 'reprocessar' | 'responsavel'>('itens');
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [showReprocessConfirmModal, setShowReprocessConfirmModal] = useState(false);
  ...
  const [manualTesApiMessage, setManualTesApiMessage] = useState<ApiMessage | null>(null);
  ...
  const handleMarcarManual = async () => {
    ...
    try {
        ...
        if (response.ok && result.status === 'ok') {
            setApiMessage({ type: 'success', text: 'Nota marcada para lançamento manual com sucesso!' });
            setTimeout(() => {
                onClose();
        ...
        console.error(error);
        setApiMessage({ type: 'error', text: error.message || 'Nao foi possivel realizar a operacao no momento, tente novamente.' });
    } finally {
        setIsSubmitting(false);
  ...
  const handleReprocessar = async () => {
    setShowReprocessConfirmModal(false);
    setIsSubmitting(true);
    setApiMessage(null);
    ...
    try {
        ...
        const result = await response.json();
        if (response.ok && result.status === 'ok') {
            setApiMessage({ type: 'success', text: 'Nota enviada para reprocessamento com sucesso!' });
            setTimeout(() => {
                onClose();
                if (onActionSuccess) onActionSuccess();
            }, 3000);
  ...
  const handleTransferirResponsavel = async () => {
    ...
    if (!selectedComprador || !motivoTransferencia.trim()) {
      setApiMessage({ type: 'error', text: 'Selecione um comprador e preencha o motivo.' });
      return;
    }
    ...
    try {
        ...
        setApiMessage({ type: 'success', text: result.message || 'Responsabilidade transferida com sucesso!' });
        setTimeout(() => {
            onClose();
            if (onActionSuccess) onActionSuccess();
        }, 3000);
        ...
        setApiMessage({ type: 'error', text: error.message || 'Não foi possível realizar a operação no momento, tente novamente.' });
    } finally {
        setIsSubmitting(false);
        setTimeout(() => setApiMessage(null), 5000);
    }
  ...
  const handleSaveChangesTes = async (updates: { id: number; tes: string }[]) => {
    ...
    try {
        ...
        const result = await response.json();
        if (response.ok && result.status === 'ok') {
            setManualTesApiMessage({ type: 'success', text: 'TES alterada com sucesso!' });
            fetchAllData(); 
        ...
        setManualTesApiMessage({ type: 'error', text: 'Não foi possível realizar a alteração nesse momento, por favor tente novamente.' });
        setIsSavingTes(false);
    }
  };
  ...
  const getJornadaActiveStep = (
      ...
      if (normalizedStatus === 'importado' || normalizedStatus === 'manual' || normalizedStatus === 'erro execauto') {
          return 5;
      }
      if (enviado) {
          return 4;
  ...
  const isButtonDisabled = statusNF?.trim().toLowerCase() === 'importado' || statusNF?.trim().toLowerCase() === 'manual';
  ...
  return (
    <>
            ...
            <div style={{flexGrow: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ flexGrow: 1, overflowY: 'scroll', padding: '0 1.5rem 1.5rem 1.5rem' }}>
                ...
                <div className="modal-tabs-container">
                  <button onClick={() => setActiveTab('itens')} className={`modal-tab-button ${activeTab === 'itens' ? 'active' : ''}`}>Itens</button>
                  ...
                  <button onClick={() => setActiveTab('reprocessar')} className={`modal-tab-button ${activeTab === 'reprocessar' ? 'active' : ''}`}>Reprocessar</button>
                ...
                <div style={{ position: 'relative', paddingTop: '1.5rem', paddingBottom: '1.5rem' }}>
                    {activeTab !== 'fiscal' && apiMessage && (
                      <div style={{ position: 'absolute', top: '10px', left: '50%', transform: 'translateX(-50%)', padding: '10px 20px', borderRadius: '5px', color: 'white', backgroundColor: apiMessage.type === 'success' ? '#28a745' : '#dc3545', zIndex: 10 }}>
                        {apiMessage.text}
                      </div>
                    )}
                    {activeTab === 'itens' && (
                                          ...
                                          <>
                                              ...
                                              {!allItemsHaveTesSuggestion && (
                                                  <div style={{ marginTop: '1.5rem' }}>
                                                      <button
                                                          onClick={() => { setManualTesApiMessage(null); setIsManualTesModalOpen(true); }}
                                                          style={{
                                                              display: 'inline-flex',
                                                              alignItems: 'center',
                                                              gap: '8px',
                                          ...
                                          <>
                                              ...
                                              <div style={{ marginTop: '1.5rem' }}>
                                                  <button
                                                      ...
                                                      disabled={isButtonDisabled || isSubmitting || apiMessage?.type === 'success'}
                                                      style={{
                                                          display: 'inline-flex',
                                                          alignItems: 'center',
                                                          padding: '10px 20px',
                                                          borderRadius: '5px',
                                                          border: 'none',
                                                          background: isButtonDisabled ? '#6c757d' : '#17a2b8',
                                                          color: 'white',
                                                          cursor: (isButtonDisabled || isSubmitting || apiMessage?.type === 'success') ? 'not-allowed' : 'pointer',
                                                          fontWeight: 'bold',
                                                          transition: 'background-color 0.2s',
                                                          opacity: (isButtonDisabled || isSubmitting || apiMessage?.type === 'success') ? 0.6 : 1
                                                      }}
                                                  >
                                                  ...
                                                  {isButtonDisabled && (
                                                      <p style={{ color: '#dc3545', fontSize: '0.9rem', marginTop: '1rem', fontWeight: 'bold' }}>
                                                          Esta opção não está disponível pois a nota já foi importada ou marcada como manual.
                                                      </p>
                                                  )}
                                              </div>
                                          </>
                    ...
                    {activeTab === 'reprocessar' && (
                       <div>
                        <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}><FaSyncAlt /> Reprocessar Nota</h3>
                        <div style={{ background: '#f8f9fa', border: '1px solid #dee2e6', borderRadius: '8px', padding: '1.5rem' }}>
                            {isSubmitting && activeTab === 'reprocessar' ? (
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1rem' }}>
                                    <div style={{ width: '30px', height: '30px', border: '3px solid #ccc', borderTop: '3px solid #1b4c89', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                                    <div style={{ marginTop: '0.5rem', fontWeight: 'bold', color: '#1b4c89' }}>Processando...</div>
                                </div>
                                ...
                                <>
                                    <p style={{ color: '#6c757d', marginTop: '0', lineHeight: 1.6 }}>
                                        Esta ação faz com que a IA analise novamente os dados da NF. A nota cairá para a fila de reprocessamento.
                                    </p>
                                    <div style={{ marginTop: '1.5rem' }}>
                                        <button
                                            onClick={() => setShowReprocessConfirmModal(true)}
                                            disabled={isButtonDisabled || isSubmitting || apiMessage?.type === 'success'}
                                            style={{
                                                padding: '10px 20px',
                                                borderRadius: '5px',
                                                border: 'none',
                                                background: isButtonDisabled ? '#6c757d' : '#f7941d',
                                                color: 'white',
                                                cursor: (isButtonDisabled || isSubmitting || apiMessage?.type === 'success') ? 'not-allowed' : 'pointer',
                                                fontWeight: 'bold',
                                                transition: 'background-color 0.2s',
                          ...
                          {isSubmitting && activeTab === 'responsavel' ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                              <CompradorSearch
                                  ...
                                  disabled={isButtonDisabled || apiMessage?.type === 'success'}
                              />
                              ...
                              <div>
                                  ...
                                  <textarea
                                      ...
                                      disabled={isSubmitting || isButtonDisabled || apiMessage?.type === 'success'}
                                  />
                              ...
                              <div>
                                  <button
                                      ...
                                      disabled={!selectedComprador || !motivoTransferencia.trim() || isSubmitting || isButtonDisabled || apiMessage?.type === 'success'}
                                      style={{
                                          padding: '10px 20px',
                                          borderRadius: '5px',
                                          border: 'none',
                                          background: (!selectedComprador || !motivoTransferencia.trim() || isButtonDisabled || apiMessage?.type === 'success') ? '#6c757d' : '#007bff',
                                          color: 'white',
                                          cursor: (!selectedComprador || !motivoTransferencia.trim() || isSubmitting || isButtonDisabled || apiMessage?.type === 'success') ? 'not-allowed' : 'pointer',
                                          fontWeight: 'bold',
                                          transition: 'background-color 0.2s',
                                          opacity: (!selectedComprador || !motivoTransferencia.trim() || isSubmitting || isButtonDisabled || apiMessage?.type === 'success') ? 0.6 : 1
                                      }}
                                  >
                                      Transferir
                                  </button>
      ...
      />
      <ConfirmationModal
        isOpen={showReprocessConfirmModal}
        onClose={() => setShowReprocessConfirmModal(false)}
        onConfirm={handleReprocessar}
        title="Aviso de Reprocessamento"
        icon={<FaInfoCircle size={40} color="#17a2b8" />}
        message="Esta ação fará com que a IA analise novamente os dados da NF e a colocará na fila de reprocessamento. Acompanhe o novo processamento na aba histórico."
        confirmText="OK, Entendi"
        confirmColor="#28a745"
      ...
      <ManualTesModal
        ...
        onClose={() => { setIsManualTesModalOpen(false); setManualTesApiMessage(null); }}
        onSave={handleSaveChangesTes}
        items={itens}
        tesData={tesData}
        isSaving={isSavingTes}
        apiMessage={manualTesApiMessage}
      />
    </>
  );
}