...
export default function PendenciaFiscal() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated') {
      ...
      const hasAccess = user?.is_admin === true || user?.funcoes?.includes('nfEntrada.pendenciaFiscal');
  ...
  const areFiltersApplied = useMemo(() => {
    ...
    const isAdvancedFiltered = advancedFilters.filial !== 'Todas' || advancedFilters.responsavel !== 'Todos';
  ...
  const fetchNotas = async () => {
      try {
        setLoading(true);
        const response = await fetch("/api/nfe/nfe-consulta-nf-pendencia-fiscal", {
          method: "POST",
        });
