...
export default function RegrasFiscaisPage() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const arrayDeRegras = (Array.isArray(data) ? data : data.regras || []).map((item: any, index: number) => {
        const operacao = (item?.operacao ?? '').toString();
        const tes = (item?.tes ?? '').toString();
        const cfop_ref_saida = (item?.cfop_ref_saida ?? '').toString();
        const cst_pis_cof_saida = Array.isArray(item?.cst_pis_cof_saida) ? item.cst_pis_cof_saida : [];
        const cst_icms_saida = Array.isArray(item?.cst_icms_saida) ? item.cst_icms_saida : [];
        return {
          ...item,
          operacao,
          tes,
  ...
  const notasFiltradasOrdenadas = useMemo(() => {
    return (regras || []).filter((regra) => {
      ...
      // Coerção defensiva
      const operacao = (regra?.operacao ?? '').toString().toLowerCase();
      const tes = (regra?.tes ?? '').toString().toLowerCase();
      const cfop = (regra?.cfop_ref_saida ?? '').toString().toLowerCase();

      const buscaOk =
        !termoBusca ||
        operacao.includes(termoBusca) ||
        tes.includes(termoBusca) ||
        cfop.includes(termoBusca);

      const tesFilter = (advancedFilters?.tes ?? '').toLowerCase();
      const cfopFilter = (advancedFilters?.cfop ?? '').toLowerCase();
      const cstFilter = (advancedFilters?.cst ?? '');

      const tesOk = !tesFilter || tes.includes(tesFilter);
      const cfopOk = !cfopFilter || cfop.includes(cfopFilter);

      const cstsPisCof = Array.isArray(regra?.cst_pis_cof_saida) ? regra.cst_pis_cof_saida : [];
      const cstOk = !cstFilter || cstsPisCof.some(c => ((c ?? '') as string).includes(cstFilter));
  ...
  const handleExportXLSX = () => {
    ...
    const data = notasFiltradasOrdenadas.map(regra => [
        ...
        regra.operacao ?? '',
        regra.tes ?? '',
        regra.cfop_ref_saida ?? '',
        regra.icms_st_nf ?? '',
        regra.icms_desonerado ?? '',
        regra.insumo_direto ?? '',
        regra.insumo_indireto ?? '',
        regra.insumo_outro ?? '',
        (Array.isArray(regra.cst_pis_cof_saida) ? regra.cst_pis_cof_saida : []).join(', '),
        (Array.isArray(regra.cst_icms_saida) ? regra.cst_icms_saida : []).join(', '),
    ]);
